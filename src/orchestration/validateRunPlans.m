...
function validateRunPlansShape(runPlans)
    ...
    % -- Validate experimentId consistency --
    experimentIds = [runPlans.experimentId];
    assert(isnumeric(experimentIds) && isvector(experimentIds), ...
        'All experimentId values must be numeric scalars.');

    assert(all(isfinite(experimentIds)) && all(experimentIds > 0) ...
        && all(mod(experimentIds, 1) == 0), ...
        'All experimentId values must be positive integers.');

    assert(all(experimentIds == experimentIds(1)), ...
        'All runPlans must share the same experimentId.');
    ...
    for i = 1:numRuns
        ...
        validateSingleMissingnessSpec(currentRunPlan.missingnessSpec, i);
...
function validateRunPlansAgainstExperimentSpec(runPlans, experimentSpec)
    ...
    numExpectedRuns = numel(experimentSpec.windowSizes) ...
        ...
        * numel(experimentSpec.missingnessSpecs) ...
    ...
    experimentIds = [runPlans.experimentId];
    assert(all(experimentIds == experimentSpec.id), ...
        ['All runPlans.experimentId values must match experimentSpec.id. ' ...
         'Expected %d.'], experimentSpec.id);
    ...
    for i = 1:numRuns
        ...
        assert(any(arrayfun(@(s) isequal(currentRunPlan.missingnessSpec, s), ...
            experimentSpec.missingnessSpecs)), ...
            ['runPlans(%d).missingnessSpec does not belong to ' ...
             'experimentSpec.missingnessSpecs.'], i);
...
function validateSingleMissingnessSpec(missingnessSpec, runIndex)
    % VALIDATESINGLEMISSINGNESSSPEC Validates a single missingness spec.
    %
    % NOTE:
    %  Schema intentionally weak for now because missingnessSpec contract
    %  is still TBD.

    assert(isstruct(missingnessSpec) && isscalar(missingnessSpec), ...
        'runPlans(%d).missingnessSpec must be a scalar struct.', runIndex);
end
...
function signatureKey = buildRunPlanSignature(runPlan)
    ...
    missingnessSpecKey = jsonencode(orderfields(runPlan.missingnessSpec));
    ...
    signatureKey = strjoin({ ...
        ...
        ['pcaSpec=' pcaSpecKey], ...
        ['missingnessSpec=' missingnessSpecKey], ...
